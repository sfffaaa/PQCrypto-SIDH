...
{ // SIKE's encryption
  ...
  // Outputs: ciphertext message ct (CRYPTO_CIPHERTEXTBYTES = CRYPTO_PUBLICKEYBYTES + MSG_BYTES bytes)
    ...
    randombytes(ct, MSG_BYTES);
    memcpy(&ct[MSG_BYTES], pk, CRYPTO_PUBLICKEYBYTES);
    cshake256_simple(ephemeralsk, SECRETKEY_A_BYTES, G, ct, CRYPTO_PUBLICKEYBYTES+MSG_BYTES);
    ...
    for (i = 0; i < MSG_BYTES; i++) ct[i + CRYPTO_PUBLICKEYBYTES] = ct[i] ^ h[i];
	return 0;
}
