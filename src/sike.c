...
{ // SIKE's encryption
  ...
  //          message m             (CRYPTO_BYTES bytes)
  // Outputs: ciphertext message ct (CRYPTO_CIPHERTEXTBYTES = CRYPTO_PUBLICKEYBYTES + MSG_BYTES bytes)
    const uint16_t P = 2;
    unsigned char sk2[SECRETKEY_A_BYTES];
    unsigned char j[FP2_ENCODED_BYTES];
    unsigned char h[MSG_BYTES];
    unsigned int i;

    randombytes(sk2, SECRETKEY_A_BYTES);

    // Encrypt
    EphemeralKeyGeneration_A(sk2, ct); //c0 set
    EphemeralSecretAgreement_A(sk2, pk, j); //j set
    cshake256_simple(h, MSG_BYTES, P, j, FP2_ENCODED_BYTES); //h set
    for (i = 0; i < MSG_BYTES; i++) ct[i + CRYPTO_PUBLICKEYBYTES] = m[i] ^ h[i];
    return 0;
}
